      ...
      child: SizedBox(
        ...
        Column(
          ...
          Row(children:  [Expanded(child: Container(
                                      ...
                                      child : const BasicButton(title: "Lancer une séance vide")))]),
          Row(children:  [Expanded(child: 
            ActionButton(action: () {
              ...
              Navigator.of(context).push(MaterialPageRoute(
                builder: ((context) => Consumer3<HistoryModel,BodyModel,ExerciceDB>(
                  builder: (context, history,body,exercices, child) 
                    => TemplateCreatorPage(
                      history:history, 
                      body:body,
                      exercices:exercices)
          ...
          ActionButton(
            ...
            action: () {
              Navigator.of(context).pop();
              Navigator.of(context).push(MaterialPageRoute(builder: ((context) => Consumer<HistoryModel>(builder: (context, value, child) => BasicAddition(history:value)))));
              },),
          )]),
          Row(children: [Expanded(child: Container(
                                      margin:const EdgeInsets.fromLTRB(20, 0, 20 ,0), 
                                      child : const BasicButton(title: "Lancer un timer")))]),
...
class _MainAppState extends State<MainApp> {
  ...
  void initState() {
    super.initState();
    context.read<CurrentExecSeance>().getPreferences();
    context.read<CurrentTemplateSeance>().getPreferences();
    context.read<ExerciceDB>().loadExs();
    var profil = context.read<Profil>();
    profil.getPreferences();
    profil.loadMusclesLevel().then((value) {
      if (profil.pseudo == null){
        showDialog(builder: (context) => PseudoDialog(),context: context);
      }
    });
  }
